...
async function actualizarStockProductos() {
  console.log('🌱 Actualizando stock de productos reales de La Fornería (PATCH)...\n');
  ...
  try {
    ...
    // Actualizar stock de productos reales usando PATCH
    console.log('📦 Actualizando stock con PATCH...\n');
    let actualizados = 0;
    ...
    for(const stock of productosReales) {
      ...
      if(producto) {
        try {
          // Solo actualizar el stock_fisico con PATCH
          ...
          console.error(`❌ Error actualizando ${stock.nombre}:`);
