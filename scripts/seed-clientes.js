...
async function seedClientes() {
  ...
  try {
    ...
    const loginResponse = await axios.post(`${API_URL}/pos/api/auth/login/`, {
      username: process.env.ADMIN_USERNAME || 'admin',
      password: process.env.ADMIN_PASSWORD || 'admin'
    });

    const token = loginResponse.data.access_token || loginResponse.data.access || loginResponse.data.key;
    ...
    console.log(`   ⚠️  Clientes existentes: ${clientesHistoricos.length - creados - errores}`);
    ...
    if (error.response?.status === 401) {
      ...
      console.error('   Puedes configurarlas con las variables de entorno:');
      console.error('   - ADMIN_USERNAME (default: admin)');
      console.error('   - ADMIN_PASSWORD (default: admin)');
    }
  }
}
